...
// Add this to the bottom of src/lib/socket.ts
export const emitAIStreamToken = (roomId: string, token: string) => {
  const io = getIO();
  io.to(`chat:${roomId}`).emit("ai:stream", { token, roomId });
};

export const emitAIStreamDone = (roomId: string) => {
  const io = getIO();
  io.to(`chat:${roomId}`).emit("ai:done", { roomId });
};

export const emitAIStreamError = (roomId: string, error: string) => {
  const io = getIO();
  io.to(`chat:${roomId}`).emit("ai:error", { roomId, error });
};

export const emitAIMessage = (roomId: string, message: any) => {
  const io = getIO();
