...
export const createChatService = async (
  ...
  const { participantId, isGroup, isAIChat, participants, groupName } = body;

  let chat;
  let allParticipantIds: string[] = [];

  // ✅ AI chat branch — no User lookup, no participantId needed
  if (isAIChat) {
    ...
    emitNewChatToParticpants([userId], populatedChat);
    return chat;
  }
